...
class GameApi {
  ...
  public joinGame(req: Request<PlayerForm>, res: Response<State>): void {
    var player = this.userService.lookup(req.body.id) as Player;
    if (
      player !== null &&
      !this.getState().currentGame.players.find(
        (playerId) => playerId === req.body.id
      )
    ) {
      this.setState(res, (state) =>
        this.gameService.addPlayer(state, player.id)
      );
    } else {
  ...
  public changeBet(req: Request<ChangeBetForm>, res: Response<State>): void {
    res.end(
      JSON.stringify(
        this.gameService.changeBet(this.getState(), req.body.id, +req.body.bet)
      )
    );
  }
